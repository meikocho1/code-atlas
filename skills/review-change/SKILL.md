...
5. **Report.** For each finding give `path:start-end` in the new file, severity (critical/high/medium/low), category (bug, security, performance, test, other), trigger, consequence, evidence, and the smallest correction or test that would verify it. Order by severity. Mark uncertainty when execution or external behavior is unverified. Do not report style preferences as defects. End with coverage: total, reviewed, and skipped files, with a reason for each skip. If no actionable issue is found, say so and name any meaningful untested area without inventing a bug.
